...
/*
*------------------------------------
*	Breif:	Generate Mottion Vector
*	param:	map<string, vector<bool>>* dictionary, string filename
*   ret  :	void
*====================================
*/
Mat coolCodec::GetMV ( Mat Ifrm, Mat Pfrm )
{
	Mat block = Ifrm ( Rect ( 0, 0, 16, 16 ) );
	cout << sum ( block );
	return Mat ( );
}
...
#endif
	imwrite ( "DCT B Image.jpg", sample[0] ( Rect ( 0, 0, Width - block_size, Height - block_size ) ) );
	imwrite ( "DCT G Image.jpg", sample[1] ( Rect ( 0, 0, Width - block_size, Height - block_size ) ) );
	imwrite ( "DCT R Image.jpg", sample[2] ( Rect ( 0, 0, Width - block_size, Height - block_size ) ) );
